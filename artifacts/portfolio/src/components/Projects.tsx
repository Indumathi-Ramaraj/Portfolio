          ...
          <AnimatePresence mode="popLayout">
            {filteredProjects.map((project) => (
              <motion.div
                ...
                key={project.id}
                className="group glass-card rounded-2xl overflow-hidden flex flex-col h-full hover:shadow-[0_0_40px_-15px_rgba(124,58,237,0.3)] transition-all duration-500"
              >
                {/* Abstract Project Image Placeholder */}
                <div className="relative h-48 w-full overflow-hidden">
                  <div className={cn("absolute inset-0 bg-gradient-to-br opacity-80 group-hover:scale-110 transition-transform duration-700", project.color)}></div>
                  ...
                  <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300">
                    <a href={project.github} className="p-2 bg-black/50 backdrop-blur-md rounded-full text-white hover:bg-primary transition-colors">
                      <Github className="w-4 h-4" />
                    </a>
                    <a href={project.live} className="p-2 bg-black/50 backdrop-blur-md rounded-full text-white hover:bg-primary transition-colors">
                ...
                <div className="p-6 flex flex-col flex-grow">
                  ...
                  <h4 className="font-display text-2xl font-bold text-white mb-3 group-hover:text-primary transition-colors">{project.title}</h4>
                  <p className="text-muted-foreground text-sm mb-6 flex-grow">{project.description}</p>
                  
                  <div className="flex flex-wrap gap-2 mt-auto">
                    {project.tech.map(t => (
                      <span key={t} className="text-xs font-medium text-white/70 bg-white/5 px-2.5 py-1 rounded border border-white/5">
                        {t}
                      </span>
                    ))}
                  </div>
